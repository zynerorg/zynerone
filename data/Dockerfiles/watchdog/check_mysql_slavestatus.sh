...
for cmd in mysql awk grep expr [; do
...
#########################################################################
while getopts "H:P:u:p:S:s:w:c:o:m:h" Input; do
  case ${Input} in
  H)
    host="-h ${OPTARG}"
    slavetarget=${OPTARG}
...
if [[ -z "${optfile}" && -z "${host}" && -z "${socket}" ]]; then
  echo -e "Missing required parameter(s)"
  exit ${STATE_UNKNOWN}
