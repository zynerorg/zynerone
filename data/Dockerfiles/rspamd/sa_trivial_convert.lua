...
local function split(str, delim)
    ...
    for token in string.gmatch(str, delim) do table.insert(result, token) end
            ...
            fun.each(function(func)
                ...
                elseif func == 'case' then
                    ...
                    rspamd_logger.warnx(rspamd_config,
                                        'Function %1 is not supported in %2',
...
local function process_sa_conf(f)
    ...
    local function insert_cur_rule()
        if not rules[cur_rule.type] then rules[cur_rule.type] = {} end

        local target = rules[cur_rule.type]

        if cur_rule.type == 'header' then
            if not cur_rule.header[1].header then
                rspamd_logger.errx(rspamd_config, 'bad rule definition: %1',
                                   cur_rule)
                return
            end
    ...
    for l in f:lines() do
        (function()
            ...
            -- Skip comments
            local words = fun.totable(fun.take_while(function(w)
                return string.sub(w, 1, 1) ~= '#'
            end, fun.filter(function(w) return w ~= "" end, fun.iter(split(l)))))

            if words[1] == "header" then
                -- header SYMBOL Header ~= /regexp/
                if valid_rule then insert_cur_rule() end
                if words[4] and (words[4] == '=~' or words[4] == '!~') then
                    ...
                    local unset_comp = string.find(cur_rule['re_expr'],
                                                   '%s+%[if%-unset:')
                    if unset_comp then
                        table.insert(complicated, l)
                        return
            ...
            elseif words[1] == "body" then
                ...
                cur_rule['symbol'] = words[2]
                if words[3] and
                    (string.sub(words[3], 1, 1) == '/' or
                        string.sub(words[3], 1, 1) == 'm') then
...
for _, matched in ipairs(arg) do
    ...
    if f then
        rspamd_logger.messagex(rspamd_config, 'loading SA rules from %s',
                               matched)
        process_sa_conf(f)
    else
...
local function handle_rule(what, syms, hdr)
    ...
    for k, r in pairs(syms) do
        local score = 0.0
        if scores[k] then score = scores[k] end
...
for k, v in pairs(rules) do
    if k == 'header' then
        for h, r in pairs(v) do handle_rule(k, r, h) end
    else
        handle_rule(k, v)
    end
end
...
fun.each(function(l)
    ...
    sa_remain:write('\n')
         ...
         fun.filter(function(l) return not string.match(l, '^%s+$') end,
                    complicated))
sa_remain:close()
