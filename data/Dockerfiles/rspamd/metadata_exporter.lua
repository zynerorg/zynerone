...
local function get_general_metadata(task, flatten, no_content)
    ...
    if fuzzy and #fuzzy > 0 then
        local fz = {}
        for _, h in ipairs(fuzzy) do table.insert(fz, h) end
        if not flatten then
            r.fuzzy = fz
        else
    ...
    if rcpt then
        local l = {}
        for _, a in ipairs(rcpt) do table.insert(l, a['addr']) end
        if not flatten then
            r.rcpt = l
        else
    ...
    if flatten then
        ...
        for _, sym in ipairs(syminf) do
            ...
            if sym.options then
                ...
                txt =
                    sym.name .. '(' .. string.format('%.2f', sym.score) .. ')' ..
                        ' [' .. topt .. ']'
            else
    ...
    local function process_header(name)
        ...
        if hdr then
            local l = {}
            for _, h in ipairs(hdr) do table.insert(l, h.decoded) end
            if not flatten then
                return l
            else
...
local formatters = {
    default = function(task) return task:get_content(), {} end,
    email_alert = function(task, rule, extra)
            ...
            if x then
                for _, e in ipairs(x) do
                    if string.len(e.addr) > 0 then
                        ...
                        table.insert(display_emails,
                                     string.format('<%s>', e.addr))
                    end
                end
            end
        end
        ...
        return
            lua_util.template(rule.email_template or settings.email_template,
                              meta), {mail_targets = mail_targets}
    end,
    json = function(task)
...
local function is_spam(action)
    return (action == 'reject' or action == 'add header' or action ==
               'rewrite subject')
end

local selectors = {
    default = function(task) return true end,
    is_spam = function(task)
        local action = task:get_metric_action('default')
        return is_spam(action)
    end,
    is_spam_authed = function(task)
        if not task:get_user() then return false end
        local action = task:get_metric_action('default')
        return is_spam(action)
    end,
        ...
        local function redis_pub_cb(err)
            if err then
                rspamd_logger.errx(task,
                                   'got error %s when publishing on server %s',
...
local process_settings = {
    select = function(val) selectors.custom = assert(load(val))() end,
    format = function(val) formatters.custom = assert(load(val))() end,
    push = function(val) pushers.custom = assert(load(val))() end,
    custom_push = function(val)
        if type(val) == 'table' then
            for k, v in pairs(val) do pushers[k] = assert(load(v))() end
        end
    end,
    ...
    pusher_enabled = function(val)
        if type(val) == 'string' then
            if pushers[val] then
                ...
                rspamd_logger.errx(rspamd_config, 'Pusher type: %s is invalid',
                                   val)
            end
...
if type(settings.rules) ~= 'table' then
    ...
    if settings.formatter then
        ...
        if not settings.format then
            rspamd_logger.errx(rspamd_config, 'No such formatter: %s',
                               settings.formatter)
            return
        end
    end
    if settings.selector then
        ...
        if not settings.select then
            rspamd_logger.errx(rspamd_config, 'No such selector: %s',
                               settings.selector)
            return
        end
    end
    for k in pairs(settings.pusher_enabled) do
        ...
        if not formatter then
            ...
            rspamd_logger.infox(rspamd_config,
                                'Using default formatter for %s pusher', k)
        else
            if not formatters[formatter] then
                rspamd_logger.errx(rspamd_config,
                                   'No such formatter: %s - disabling %s',
                                   formatter, k)
        ...
        if not selector then
            ...
            rspamd_logger.infox(rspamd_config,
                                'Using default selector for %s pusher', k)
        else
            if not selectors[selector] then
                rspamd_logger.errx(rspamd_config,
                                   'No such selector: %s - disabling %s',
                                   selector, k)
                settings.pusher_enabled.k = nil
            end
        end
    ...
    if settings.pusher_enabled.send_mail then
        if not (settings.mail_to and settings.smtp) then
            rspamd_logger.errx(rspamd_config,
                               'No mail_to and/or smtp setting is specified')
...
local check_element = {
    selector = function(k, v)
        if not selectors[v] then
            rspamd_logger.errx(rspamd_config, 'Rule %s has invalid selector %s',
                               k, v)
            return false
        else
            return true
    ...
    formatter = function(k, v)
        if not formatters[v] then
            rspamd_logger.errx(rspamd_config,
                               'Rule %s has invalid formatter %s', k, v)
            return false
        else
            return true
        end
    ...
    default = function(k, rule)
        ...
        if reqset then
            for _, e in ipairs(reqset) do
                if not rule[e] then
                    rspamd_logger.errx(rspamd_config,
                                       'Rule %s misses required setting %s', k,
                                       e)
        ...
        for sett, v in pairs(rule) do
            ...
            if f then
                if not f(sett, v) then settings.rules[k] = nil end
            end
        end
    end
}
backend_check.redis_pubsub = function(k, rule)
    if not redis_params then redis_params = rspamd_parse_redis_server(N) end
    if not redis_params then
        rspamd_logger.errx(rspamd_config, 'No redis servers are specified')
        settings.rules[k] = nil
    else
...
setmetatable(backend_check,
             {__index = function() return backend_check.default end})
for k, v in pairs(settings.rules) do
    if type(v) == 'table' then
        ...
        elseif not pushers[backend] then
            rspamd_logger.errx(rspamd_config, 'Rule %s has invalid backend %s',
                               k, backend)
            settings.rules[k] = nil
        else
        ...
        if selected then
            ...
            if formatted then
                ...
                lua_util.debugm(N, task,
                                'Formatter [%s] returned non-truthy value [%s]',
                                formatter, formatted)
            end
        else
            lua_util.debugm(N, task,
                            'Selector [%s] returned non-truthy value [%s]',
                            selector, selected)
        end
    end
end
