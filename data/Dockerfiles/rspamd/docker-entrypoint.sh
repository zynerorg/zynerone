...
[[ ! -f /etc/rspamd/override.d/worker-controller-password.inc ]] && echo '# Autogenerated by zynerone' > /etc/rspamd/override.d/worker-controller-password.inc

echo ${IPV4_NETWORK}.0/24 > /etc/rspamd/custom/zynerone_networks.map
echo ${IPV6_NETWORK} >> /etc/rspamd/custom/zynerone_networks.map
...
touch /etc/rspamd/custom/global_mime_from_blacklist.map \
  ...
  /etc/rspamd/custom/zynerone_networks.map \
