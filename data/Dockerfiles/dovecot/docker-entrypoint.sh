...
until dig +short zyner.one >/dev/null; do
...
rm -rf /var/vmail/zynerone.local/*
# Cleanup PIDs
...
echo ${RAND_USER}@zynerone.local:{SHA1}$(echo -n ${RAND_PASS} | sha1sum | awk '{print $1}'):::::: >/etc/dovecot/dovecot-master.passwd
echo ${RAND_USER}@zynerone.local::5000:5000:::: >/etc/dovecot/dovecot-master.userdb
echo ${RAND_USER}@zynerone.local:${RAND_PASS} >/etc/sogo/sieve.creds
