...
jQuery(function ($) {
  ...
  function unix_time_format(i) {
    return "" == i
      ? '<i class="bi bi-x"></i>'
      : new Date(i ? 1e3 * i : 0).toLocaleDateString(void 0, {
          year: "numeric",
          month: "2-digit",
          day: "2-digit",
          hour: "2-digit",
          ...
          $.each(json, function (i, item) {
            ...
            if (item.backupmx == 1) {
              if (item.relay_unknown_only == 1) {
                item.domain_name =
                  '<div class="badge fs-6 bg-info">Relay Non-Local</div> ' +
                  item.domain_name;
              } else if (item.relay_all_recipients == 1) {
                item.domain_name =
                  '<div class="badge fs-6 bg-info">Relay All</div> ' +
                  item.domain_name;
              } else {
                item.domain_name =
                  '<div class="badge fs-6 bg-info">Relay</div> ' +
                  item.domain_name;
              }
            }
          });
        ...
        dataSrc: function (json) {
          $.each(json, function (i, item) {
            ...
            item.last_mail_login =
              item.last_imap_login +
              "/" +
              item.last_pop3_login +
              "/" +
              item.last_smtp_login;
            /*
            if (!item.rl) {
              item.rl = '∞';
            } else {
            ...
            if (acl_data.login_as === 1) {
              ...
              if (ALLOW_ADMIN_EMAIL_LOGIN) {
                item.action +=
                  ...
                  encodeURIComponent(item.username) +
                  '" class="login_as btn btn-sm btn-xs-lg btn-xs-half btn-primary" target="_blank"><i class="bi bi-envelope-fill"></i> SOGo</a>';
              }
              item.action += "</div>";
            } else {
              item.action =
      ...
      columns: [
        ...
        },
        {
          ...
          render: function (data, type) {
            res = data.split("/");
            return (
              '<div class="badge bg-info mb-2">IMAP @ ' +
              unix_time_format(Number(res[0])) +
              "</div><br>" +
              '<div class="badge bg-info mb-2">POP3 @ ' +
              unix_time_format(Number(res[1])) +
              "</div><br>" +
              '<div class="badge bg-info">SMTP @ ' +
              unix_time_format(Number(res[2])) +
              "</div>"
            );
          },
        },
