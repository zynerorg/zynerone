...
jQuery(function ($) {
  ...
  function draw_quarantine_table() {
    var table = $("#quarantinetable").DataTable({
      ...
      aColumns: [{ sWidth: "8.25%" }, { sClass: "classDataTable" }],
      dom:
          ...
          $.each(data, function (i, item) {
            ...
            if (acl_data.login_as === 1) {
              item.action =
                ...
                '<a href="#" data-item="' +
                encodeURI(item.id) +
                '" class="btn btn-xs btn-xs-half btn-info show_qid_info"><i class="bi bi-box-arrow-up-right"></i> ' +
                lang.show_item +
                "</a>" +
                '<a href="#" data-action="delete_selected" data-id="del-single-qitem" data-api-url="delete/qitem" data-item="' +
                encodeURI(item.id) +
              ...
              item.action =
                ...
                '<a href="#" data-item="' +
                encodeURI(item.id) +
                '" class="btn btn-xs btn-info show_qid_info"><i class="bi bi-file-earmark-text"></i> ' +
                lang.show_item +
                "</a>" +
                "</div>";
            }
            item.chkbox =
              '<input type="checkbox" class="form-check-input" data-id="qitems" name="multi_select" value="' +
              item.id +
              '" />';
          });
  ...
  $("body").on("click", ".show_qid_info", function (e) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#quick_download_link").attr(
          "onclick",
          "window.open('/inc/ajax/qitem_details.php?id=" +
            qitem +
            "&eml', '_blank')"
        );
        $("#quick_release_link").attr(
          "onclick",
          "window.open('/inc/ajax/qitem_details.php?id=" +
            qitem +
            "&quick_release', '_blank')"
        );
        $("#quick_delete_link").attr(
          "onclick",
          "window.open('/inc/ajax/qitem_details.php?id=" +
            qitem +
            "&quick_delete', '_blank')"
        ...
        if (typeof data.symbols !== "undefined") {
          ...
          $.each(data.symbols, function (index, value) {
            ...
            $("#qid_detail_symbols").append(
              '<span data-bs-toggle="tooltip" class="rspamd-symbol ' +
                highlightClass +
                '" title="' +
                (value.options ? escapeHtml(value.options.join(", ")) : "") +
                '">' +
                value.name +
                ' (<span class="score">' +
                value.score +
        ...
        if (
          typeof data.fuzzy_hashes === "object" &&
          data.fuzzy_hashes !== null &&
          data.fuzzy_hashes.length !== 0
        ) {
          $.each(data.fuzzy_hashes, function (index, value) {
            $("#qid_detail_fuzzy").append(
              '<p style="font-family:monospace">' + value + "</p>"
            );
          });
        } else {
          $("#qid_detail_fuzzy").append("-");
        ...
        if (
          ...
          if (data.action == "add header") {
            $("#qid_detail_score").append(
              '<span class="label-rspamd-action badge fs-6 bg-warning"><b>' +
                data.score +
                "</b> - " +
                lang.junk_folder +
                "</span>"
            ...
            $("#qid_detail_score").append(
              '<span class="label-rspamd-action badge fs-6 bg-danger"><b>' +
                data.score +
                "</b> - " +
                lang.rejected +
                "</span>"
        ...
        if (typeof data.attachments !== "undefined") {
          ...
          $.each(data.attachments, function (index, value) {
            qAtts.append(
              '<p><a href="/inc/ajax/qitem_details.php?id=' +
                qitem +
                "&att=" +
                index +
                '" target="_blank">' +
  ...
  function hideTableExpandCollapseBtn(table) {
    if ($(table).hasClass("collapsed")) $(".table_collapse_option").show();
    else $(".table_collapse_option").hide();
  }
});
