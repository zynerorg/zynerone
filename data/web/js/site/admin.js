...
jQuery(function ($) {
  ...
  function validateRegex(e) {
    var t = e.split("/"),
      n = e,
      r = "";
    t.length > 1 && ((n = t[1]), (r = t[2]));
    try {
      return new RegExp(n, r), !0;
    } catch (e) {
      return !1;
    }
  }
  ...
  function intToRGB(t) {
    var n = (16777215 & t).toString(16).toUpperCase();
  ...
  $(".arrow-toggle").on("click", function (e) {
    e.preventDefault();
    $(this).find(".arrow").toggleClass("animation");
  });
  ...
  $(".admin-ays-dialog").click(function () {
    return confirm(lang.ays);
  });
  ...
  $("#super_delete").click(function () {
    return confirm(lang.queue_ays);
  });
      ...
      columns: [
        ...
        },
        {
          ...
          render: function (data, type) {
            if (data == 1)
              return '<i class="bi bi-check-lg"><span class="sorting-value">1</span></i>';
            else
              return '<i class="bi bi-x-lg"><span class="sorting-value">0</span></i>';
          },
        },
        {
          title: lang.active,
    ...
    $("#forwardinghoststable").DataTable({
      ...
      columns: [
        ...
        },
        {
          ...
          render: function (data, type) {
            return "yes" == data
              ? '<i class="bi bi-x-lg"><span class="sorting-value">yes</span></i>'
              : "no" == data &&
                  '<i class="bi bi-check-lg"><span class="sorting-value">no</span></i>';
          },
        },
        {
          title: lang.action,
  ...
  function process_table_data(data, table) {
    if (table == "relayhoststable") {
      $.each(data, function (i, item) {
        item.action =
          '<div class="btn-group">' +
          '<a href="#" data-bs-toggle="modal" data-bs-target="#testTransportModal" data-transport-id="' +
          encodeURI(item.id) +
          '" data-transport-type="sender-dependent" class="btn btn-xs btn-xs-lg btn-xs-third btn-secondary"><i class="bi bi-caret-right-fill"></i> Test</a>' +
          '<a href="/edit/relayhost/' +
          encodeURI(item.id) +
          '" class="btn btn-xs btn-xs-lg btn-xs-third btn-secondary"><i class="bi bi-pencil-fill"></i> ' +
          lang.edit +
          "</a>" +
          '<a href="#" data-action="delete_selected" data-id="single-rlyhost" data-api-url="delete/relayhost" data-item="' +
          encodeURI(item.id) +
          '" class="btn btn-xs btn-xs-lg btn-xs-third btn-danger"><i class="bi bi-trash"></i> ' +
          lang.remove +
          "</a>" +
          "</div>";
        if (item.used_by_mailboxes == "") {
          item.in_use_by = item.used_by_domains;
        } else if (item.used_by_domains == "") {
          item.in_use_by = item.used_by_mailboxes;
        } else {
          item.in_use_by =
            item.used_by_mailboxes +
            '<hr style="margin:5px 0px 5px 0px;">' +
            item.used_by_domains;
        }
        item.chkbox =
          '<input type="checkbox" class="form-check-input" data-id="rlyhosts" name="multi_select" value="' +
          item.id +
          '" />';
      });
      ...
      $.each(data, function (i, item) {
        if (item.is_mx_based) {
          item.destination =
            '<i class="bi bi-info-circle-fill text-info mx-info" data-bs-toggle="tooltip" title="' +
            lang.is_mx_based +
            '"></i> <code>' +
            item.destination +
            "</code>";
        ...
        if (item.username) {
          item.username =
            '<i style="color:#' +
            intToRGB(hashCode(item.nexthop)) +
            ';" class="bi bi-square-fill"></i> ' +
            item.username;
        }
        item.action =
          '<div class="btn-group">' +
          '<a href="#" data-bs-toggle="modal" data-bs-target="#testTransportModal" data-transport-id="' +
          encodeURI(item.id) +
          '" data-transport-type="transport-map" class="btn btn-xs btn-xs-lg btn-xs-third btn-secondary"><i class="bi bi-caret-right-fill"></i> Test</a>' +
          '<a href="/edit/transport/' +
          encodeURI(item.id) +
          '" class="btn btn-xs btn-xs-lg btn-xs-third btn-secondary"><i class="bi bi-pencil-fill"></i> ' +
          lang.edit +
          "</a>" +
          '<a href="#" data-action="delete_selected" data-id="single-transport" data-api-url="delete/transport" data-item="' +
          encodeURI(item.id) +
          '" class="btn btn-xs btn-xs-lg btn-xs-third btn-danger"><i class="bi bi-trash"></i> ' +
          lang.remove +
          "</a>" +
          "</div>";
        item.chkbox =
          '<input type="checkbox" class="form-check-input" data-id="transports" name="multi_select" value="' +
          item.id +
          '" />';
      });
      ...
      $.each(data, function (i, item) {
        item.chkbox =
          '<input type="checkbox" class="form-check-input" data-id="mailqitems" name="multi_select" value="' +
          item.queue_id +
          '" />';
        ...
        item.action =
          '<div class="btn-group">' +
          '<a href="#" data-bs-toggle="modal" data-bs-target="#showQueuedMsg" data-queue-id="' +
          encodeURI(item.queue_id) +
          '" class="btn btn-xs btn-xs-lg btn-secondary">' +
      ...
      $.each(data, function (i, item) {
        item.action =
          '<div class="btn-group">' +
          '<a href="#" data-action="delete_selected" data-id="single-fwdhost" data-api-url="delete/fwdhost" data-item="' +
          encodeURI(item.host) +
          '" class="btn btn-xs btn-xs-lg btn-danger"><i class="bi bi-trash"></i> ' +
          lang.remove +
          "</a>" +
          "</div>";
        item.chkbox =
          '<input type="checkbox" class="form-check-input" data-id="fwdhosts" name="multi_select" value="' +
          item.host +
          '" />';
      });
      ...
      $.each(data, function (i, item) {
        item.action =
          '<div class="btn-group">' +
          '<a href="/edit.php?oauth2client=' +
          encodeURI(item.id) +
          '" class="btn btn-xs btn-xs-lg btn-xs-half btn-secondary"><i class="bi bi-pencil-fill"></i> ' +
          lang.edit +
          "</a>" +
          '<a href="#" data-action="delete_selected" data-id="single-oauth2-client" data-api-url="delete/oauth2-client" data-item="' +
          encodeURI(item.id) +
          '" class="btn btn-xs btn-xs-lg btn-xs-half btn-danger"><i class="bi bi-trash"></i> ' +
          lang.remove +
          "</a>" +
          "</div>";
        item.scope = "profile";
        ...
        item.chkbox =
          '<input type="checkbox" class="form-check-input" data-id="oauth2_clients" name="multi_select" value="' +
          item.id +
          '" />';
      });
      ...
      $.each(data, function (i, item) {
        ...
        item.selected_domains = item.selected_domains
          .toString()
          .replace(/,/g, "<br>");
        item.chkbox =
          '<input type="checkbox" class="form-check-input" data-id="domain_admins" name="multi_select" value="' +
          item.username +
          '" />';
        item.action =
          '<div class="btn-group">' +
          '<a href="/edit/domainadmin/' +
          encodeURI(item.username) +
          '" class="btn btn-xs btn-xs-lg btn-xs-third btn-secondary"><i class="bi bi-pencil-fill"></i> ' +
          lang.edit +
          "</a>" +
          '<a href="#" data-action="delete_selected" data-id="single-domain-admin" data-api-url="delete/domain-admin" data-item="' +
          encodeURI(item.username) +
          '" class="btn btn-xs btn-xs-lg btn-xs-third btn-danger"><i class="bi bi-trash"></i> ' +
          lang.remove +
          "</a>" +
          ...
          '" class="btn btn-xs btn-xs-lg btn-xs-third btn-success"><i class="bi bi-person-fill"></i> Login</a>' +
          "</div>";
      });
      ...
      $.each(data, function (i, item) {
        ...
        item.chkbox =
          '<input type="checkbox" class="form-check-input" data-id="admins" name="multi_select" value="' +
          item.username +
          '" />';
        item.action =
          '<div class="btn-group">' +
          ...
          lang.edit +
          "</a>" +
          '<a href="#" data-action="delete_selected" data-id="single-admin" data-api-url="delete/admin" data-item="' +
          encodeURI(item.username) +
          '" class="btn btn-xs btn-xs-lg btn-xs-half btn-danger"><i class="bi bi-trash"></i> ' +
          lang.remove +
          "</a>" +
  ...
  function add_table_row(table_id, type) {
    ...
    if (type == "app_link") {
      cols =
        '<td><input class="input-sm input-xs-lg form-control" data-id="app_links" type="text" name="app" required></td>';
      cols +=
        '<td><input class="input-sm input-xs-lg form-control" data-id="app_links" type="text" name="href" required></td>';
      cols +=
      ...
      cols =
        '<td><input style="text-align:center" class="input-sm input-xs-lg form-control" data-id="f2b_regex" type="text" value="+" disabled></td>';
      cols +=
        '<td><input class="input-sm input-xs-lg form-control regex-input" data-id="f2b_regex" type="text" name="regex" required></td>';
      cols +=
        '<td><a href="#" role="button" class="btn btn-sm btn-xs-lg btn-secondary h-100 w-100" type="button">' +
        lang.remove_row +
        "</a></td>";
