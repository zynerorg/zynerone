$(document).ready(function () {
  ...
  $(".parse_s_ago").each(function (i, parse_s_ago) {
    ...
    if (typeof started_s_ago != "NaN") {
      ...
      if (started_date instanceof Date && !isNaN(started_date)) {
        var started_local_date = started_date.toLocaleDateString(
          LOCALE,
          DATETIME_FORMAT
        );
  ...
  $("#zynerone_version").click(function () {
    if (
      (zynerone_cc_role !== "admin" && zynerone_cc_role !== "domainadmin") ||
      zynerone_info.branch !== "master"
    )
      return;

    showVersionModal(
      "Version " + zynerone_info.version_tag,
      zynerone_info.version_tag
    );
  });
  // get public ips
  $("#host_show_ip").click(function () {
    ...
    window
      .fetch("/api/v1/get/status/host/ip", { method: "GET", cache: "no-cache" })
      ...
      .then(function (data) {
        ...
        // display host ips
        if (data.ipv4) $("#host_ipv4").text(data.ipv4);
        if (data.ipv6) $("#host_ipv6").text(data.ipv6);
...
jQuery(function ($) {
    ...
    var table = $("#sasl_logs").DataTable({
      ...
      columns: [
        ...
        },
        {
          ...
          createdCell: function (td, cellData) {
            cellData = Math.floor(
              new Date(cellData.replace(/-/g, "/")).getTime() / 1000
            );
            createSortableDate(td, cellData);
          },
        },
    ...
    if (table == "rspamd_history") {
      $.each(data, function (i, item) {
        ...
        item.symbols = Object.keys(item.symbols)
          ...
          .map(function (key) {
            ...
            if (sym.score < 0) {
              sym.score_formatted =
                '(<span class="text-success"><b>' + sym.score + "</b></span>)";
            } else if (sym.score === 0) {
              sym.score_formatted = "(<span><b>" + sym.score + "</b></span>)";
            } else {
              sym.score_formatted =
                '(<span class="text-danger"><b>' + sym.score + "</b></span>)";
            }
        ...
        if (item.action === "clean" || item.action === "no action") {
          item.action =
            "<div class='badge fs-6 bg-success'>" + item.action + "</div>";
        } else if (
          item.action === "rewrite subject" ||
          item.action === "add header" ||
          item.action === "probable spam"
        ) {
          item.action =
            "<div class='badge fs-6 bg-warning'>" + item.action + "</div>";
        } else if (item.action === "spam" || item.action === "reject") {
          item.action =
            "<div class='badge fs-6 bg-danger'>" + item.action + "</div>";
        } else {
          item.action =
            "<div class='badge fs-6 bg-info'>" + item.action + "</div>";
        }
        var score_content;
      ...
      $.each(data, function (i, item) {
        ...
        if (item.service == "activesync") {
          ...
          item.service =
            '<span class="badge fs-6 bg-success">IMAP, SMTP, Cal-/CardDAV</span>';
        } else {
          item.service =
      ...
      $.each(data, function (i, item) {
        if (item.message == null) {
          ...
          if (item.hpdiff < 0) {
            item.trend =
              '<span class="badge fs-6 bg-danger"><i class="bi bi-caret-down-fill"></i> ' +
              item.hpdiff +
              "</span>";
          } else if (item.hpdiff == 0) {
            item.trend =
              '<span class="badge fs-6 bg-info"><i class="bi bi-caret-right-fill"></i> ' +
              item.hpdiff +
              "</span>";
          } else {
            item.trend =
              '<span class="badge fs-6 bg-success"><i class="bi bi-caret-up-fill"></i> ' +
              item.hpdiff +
              "</span>";
          }
        } else {
      ...
      $.each(data, function (i, item) {
        ...
        if (jQuery.inArray(item.priority, danger_class) !== -1) {
          item.priority =
            '<span class="badge fs-6 bg-danger">' + item.priority + "</span>";
        } else if (jQuery.inArray(item.priority, warning_class) !== -1) {
          item.priority =
            '<span class="badge fs-6 bg-warning">' + item.priority + "</span>";
        } else if (jQuery.inArray(item.priority, info_class) !== -1) {
          item.priority =
            '<span class="badge fs-6 bg-info">' + item.priority + "</span>";
        }
      });
      ...
      $.each(data, function (i, item) {
        ...
        if (item.method == "GET") {
          item.method =
            '<span class="badge fs-6 bg-success">' + item.method + "</span>";
        } else if (item.method == "POST") {
          item.method =
            '<span class="badge fs-6 bg-warning">' + item.method + "</span>";
        }
      ...
      $.each(data, function (i, item) {
        ...
        item.indicator =
          '<span style="border-right:6px solid #' +
          intToRGB(hashCode(item.rl_hash)) +
          ';padding-left:5px;">&nbsp;</span>';
        if (item.rl_hash != "err") {
          item.action =
            '<a href="#" data-action="delete_selected" data-id="single-hash" data-api-url="delete/rlhash" data-item="' +
            encodeURI(item.rl_hash) +
            '" class="btn btn-xs btn-danger"><i class="bi bi-trash"></i> ' +
            lang.reset_limit +
            "</a>";
        }
      });
  ...
  $(".add_log_lines").on("click", function (e) {
    ...
    if (
      ...
      console.log(
        "no data-table or data-nrows or log_url or data-post-process attr found"
      );
      return;
    }

    if ((table = $("#" + log_table).DataTable())) {
      var heading = $("#" + log_table)
        .closest(".card")
        .find(".card-header");
      var load_rows =
        table.data().count() + 1 + "-" + (table.data().count() + new_nrows);

      $.get("/api/v1/get/logs/" + log_url + "/" + load_rows).then(function (
        data
      ) {
        if (data.length === undefined) {
          zynerone_alert_box(lang.no_new_rows, "info");
          return;
        }
      ...
      entries.forEach((entry) => {
        if (entry.intersectionRatio > 0) {
          if (!containerElements[i].classList.contains("show")) {
            ...
            containerElements[i].addEventListener(
              "hidden.bs.collapse",
              function () {
...
function update_stats(timeout = 5) {
  ...
  window
    .fetch("/api/v1/get/status/host", { method: "GET", cache: "no-cache" })
    .then(function (response) {
      return response.json();
    })
    .then(function (data) {
      ...
      if (data) {
        ...
        $("#host_memory_total").text(
          (data.memory.total / 1024 ** 3).toFixed(2).toString() + "GB"
        );
        $("#host_memory_usage").text(
          parseInt(data.memory.usage).toString() + "%"
        );
        if (data.architecture == "aarch64") {
          $("#host_architecture").html(
        ...
        if (cpu_chart.data.datasets[0].data.length > 30)
          cpu_chart.data.datasets[0].data.shift();
        mem_chart.data.datasets[0].data.push(data.memory.usage);
        if (mem_chart.data.datasets[0].data.length > 30)
          mem_chart.data.datasets[0].data.shift();
...
function update_container_stats(timeout = 5) {
  if ($("#tab-containers").hasClass("active")) {
    for (let container in containersToUpdate) {
      ...
      if (containersToUpdate[container].state == "running") continue;
      containersToUpdate[container].state = "running";

      window
        .fetch("/api/v1/get/status/container/" + container_id, {
          method: "GET",
          cache: "no-cache",
        })
        .then(function (response) {
        ...
        .then(function (data) {
          ...
          if (data.length >= 2) {
            ...
            $("#" + container + "_DiskIOChart")
              .prev()
              .addClass("d-none");
            $("#" + container + "_NetIOChart").removeClass("d-none");
            $("#" + container + "_NetIOChart")
              .prev()
              .addClass("d-none");
          }
          ...
          if (prev_stats != null) {
            // calc time diff
            var time_diff =
              (new Date(data.read) - new Date(prev_stats.read)) / 1000;

            // calc disk io b/s
            if (
              "io_service_bytes_recursive" in prev_stats.blkio_stats &&
              prev_stats.blkio_stats.io_service_bytes_recursive !== null
            ) {
              var prev_read_bytes = 0;
              var prev_write_bytes = 0;
              for (
                var i = 0;
                i < prev_stats.blkio_stats.io_service_bytes_recursive.length;
                i++
              ) {
                if (
                  prev_stats.blkio_stats.io_service_bytes_recursive[i].op ==
                  "read"
                )
                  prev_read_bytes =
                    prev_stats.blkio_stats.io_service_bytes_recursive[i].value;
                else if (
                  prev_stats.blkio_stats.io_service_bytes_recursive[i].op ==
                  "write"
                )
              ...
              for (
                var i = 0;
                i < data.blkio_stats.io_service_bytes_recursive.length;
                i++
              ) {
                if (data.blkio_stats.io_service_bytes_recursive[i].op == "read")
                  read_bytes =
                    data.blkio_stats.io_service_bytes_recursive[i].value;
                else if (
                  data.blkio_stats.io_service_bytes_recursive[i].op == "write"
                )
                  write_bytes =
                    data.blkio_stats.io_service_bytes_recursive[i].value;
              }
              var diff_bytes_read = (read_bytes - prev_read_bytes) / time_diff;
              var diff_bytes_write =
                (write_bytes - prev_write_bytes) / time_diff;
            }
...
function addReadWriteChart(
  ...
  if (chart_context.data.labels.length > limit)
    chart_context.data.labels.shift();
  ...
  if (chart_context.data.datasets[0].data.length > limit)
    chart_context.data.datasets[0].data.shift();
  if (chart_context.data.datasets[1].data.length > limit)
    chart_context.data.datasets[1].data.shift();
      ...
      window
        ...
        .then(function (current_data) {
          ...
          if (date_latest.getTime() <= date_current.getTime()) {
            ...
            $("#zynerone_update").html(
              "<b>" + lang_debug.no_update_available + "</b>"
            );
          } else {
            // update available
            $("#zynerone_update")
            ...
            $("#zynerone_update").html(
              lang_debug.update_available +
                ` <a href="#" id="zynerone_update_changelog">` +
                latest_data.tag_name +
                `</a>`
            );
            $("#zynerone_update_changelog").click(function () {
              if (
                zynerone_cc_role !== "admin" &&
                zynerone_cc_role !== "domainadmin"
              )
                return;

              showVersionModal(
                "New Release " + latest_data.tag_name,
                latest_data.tag_name
              );
            });
          }
...
function showVersionModal(title, version) {
  $.ajax({
    ...
    url:
      "https://api.github.com/repos/" +
      zynerone_info.project_owner +
      "/" +
      zynerone_info.project_repo +
      "/releases/tags/" +
      version,
    dataType: "json",
    success: function (data) {
...
function parseGithubMarkdownLinks(inputText) {
  ...
  replacedText = inputText.replace(
    replacePattern1,
    (matched, index, original, input_string) => {
      if (matched.includes("github.com")) {
        ...
        if (
          matched.includes("/compare/") &&
          zynerone_info.last_version_tag !== ""
        ) {
          matched = matched.replace(
            last_uri_path,
            zynerone_info.last_version_tag + "..." + zynerone_info.version_tag
          );
          last_uri_path =
            zynerone_info.last_version_tag + "..." + zynerone_info.version_tag;
        }
