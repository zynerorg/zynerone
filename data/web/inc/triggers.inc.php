...
if (!empty($_GET['sso_token'])) {
  ...
  if ($username !== false) {
    $_SESSION['zynerone_cc_username'] = $username;
...
if (isset($_POST["verify_tfa_login"])) {
  if (verify_tfa_login($_SESSION['pending_zynerone_cc_username'], $_POST)) {
    $_SESSION['zynerone_cc_username'] = $_SESSION['pending_zynerone_cc_username'];
    $_SESSION['zynerone_cc_role'] = $_SESSION['pending_zynerone_cc_role'];
    unset($_SESSION['pending_zynerone_cc_username']);
    unset($_SESSION['pending_zynerone_cc_role']);
...
if (isset($_POST["login_user"]) && isset($_POST["pass_user"])) {
	...
	if ($as == "admin") {
		$_SESSION['zynerone_cc_username'] = $login_user;
    ...
    unset($_SESSION['pending_zynerone_cc_role']);
    unset($_SESSION['pending_tfa_methods']);
		unset($_SESSION['zynerone_cc_username']);
		unset($_SESSION['zynerone_cc_role']);
	}
}

if (isset($_SESSION['zynerone_cc_role']) && (isset($_SESSION['acl']['login_as']) && $_SESSION['acl']['login_as'] == "1")) {
    ...
    if (filter_var($duallogin, FILTER_VALIDATE_EMAIL)) {
      if (!empty(mailbox('get', 'mailbox_details', $duallogin))) {
        $_SESSION["dual-login"]["username"] = $_SESSION['zynerone_cc_username'];
        $_SESSION["dual-login"]["role"]     = $_SESSION['zynerone_cc_role'];
        $_SESSION['zynerone_cc_username']    = $duallogin;
        $_SESSION['zynerone_cc_role']        = "user";
        header("Location: /user");
      }
    }
...
if (isset($_SESSION['zynerone_cc_role']) && $_SESSION['zynerone_cc_role'] == "admin" && !isset($_SESSION['zynerone_cc_api'])) {
