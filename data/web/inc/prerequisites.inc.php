...
if (getenv('WEBAUTHN_ONLY_TRUSTED_VENDORS') == 'y')
  $WebAuthn->addRootCertificates($_SERVER['DOCUMENT_ROOT'] . '/inc/lib/WebAuthn/rootCertificates');
...
// OAuth2
class zyneronePdo extends OAuth2\Storage\Pdo
{
...
$oauth2_storage = new zyneronePdo(array('dsn' => $dsn, 'username' => $database_user, 'password' => $database_pass));
...
// Set language
if (!isset($_SESSION['zynerone_locale']) && !isset($_COOKIE['zynerone_locale'])) {
  if ($DETECT_LANGUAGE && isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
    ...
    foreach ($lang2pref as $lang => $q) {
      if (array_key_exists($lang, $AVAILABLE_LANGUAGES)) {
        $_SESSION['zynerone_locale'] = $lang;
        break;
      } elseif (array_key_exists($lang, $AVAILABLE_BASE_LANGUAGES)) {
        $_SESSION['zynerone_locale'] = $AVAILABLE_BASE_LANGUAGES[$lang];
        break;
      }
    }
    ...
    if (!isset($_SESSION['zynerone_locale'])) {
      foreach ($lang2pref as $lang => $q) {
        if (array_key_exists(substr($lang, 0, 2), $AVAILABLE_BASE_LANGUAGES)) {
          $_SESSION['zynerone_locale'] = $AVAILABLE_BASE_LANGUAGES[substr($lang, 0, 2)];
          break;
        }
      }
    }
  } else {
    $_SESSION['zynerone_locale'] = strtolower(trim($DEFAULT_LANG));
  }
}
if (isset($_COOKIE['zynerone_locale'])) {
  (preg_match('/^[a-z]{2}$/', $_COOKIE['zynerone_locale'])) ? $_SESSION['zynerone_locale'] = $_COOKIE['zynerone_locale'] : setcookie("zynerone_locale", "", time() - 300);
}
if (isset($_GET['lang']) && array_key_exists($_GET['lang'], $AVAILABLE_LANGUAGES)) {
  $_SESSION['zynerone_locale'] = $_GET['lang'];
  setcookie("zynerone_locale", $_GET['lang'], time() + 30758400); // one year
}
...
$langFile = $_SERVER['DOCUMENT_ROOT'] . '/lang/lang.' . $_SESSION['zynerone_locale'] . '.json';
...
if (isset($_SESSION['zynerone_cc_role'])) {
  ...
  // $imap_connection = $imap_server->authenticate($_SESSION['zynerone_cc_username'] . '*' . trim($master_user), trim($master_passwd));
