...
rspamd_config:register_symbol({
    ...
    callback = function(task)
        ...
        if not limited_access then return false end
        ...
        if ip == nil or not ip:is_valid() then return false end
        ...
        local function smtp_access_cb(err, data)
            if err then
                rspamd_logger.infox(rspamd_config,
                                    "smtp_access query request for ip %s returned invalid or empty data (\"%s\") or error (\"%s\")",
                                    ip, data, err)
                return false
            else
                rspamd_logger.infox(rspamd_config,
                                    "checking ip %s for smtp_access in %s",
                                    from_ip_string, hash_key)
                for k, v in pairs(data) do
                    if (v and v ~= userdata and v == '1') then
                        rspamd_logger.infox(rspamd_config,
                                            "found ip in smtp_access map")
                        task:insert_result(true, 'SMTP_ACCESS', 0.0,
                                           from_ip_string)
                        return true
                    end
                end
                rspamd_logger.infox(rspamd_config,
                                    "couldnt find ip in smtp_access map")
                task:insert_result(true, 'SMTP_ACCESS', 999.0, from_ip_string)
                return true
            end
        end
        table.insert(smtp_access_table, 1, hash_key)
        local redis_ret_user =
            rspamd_redis_make_request(task, redis_params, -- connect params
            hash_key, -- hash key
            false, -- is write
        ...
        if not redis_ret_user then
            rspamd_logger.infox(rspamd_config,
                                "cannot check smtp_access redis map")
        end
    end,
    priority = 10
})
...
rspamd_config:register_symbol({
    ...
    callback = function(task)
        ...
        if uname then return false end
        ...
        local function keep_spam_cb(err, data)
            if err then
                rspamd_logger.infox(rspamd_config,
                                    "keep_spam query request for ip %s returned invalid or empty data (\"%s\") or error (\"%s\")",
                                    ip, data, err)
                return false
            else
                for k, v in pairs(data) do
                    if (v and v ~= userdata and v == '1') then
                        rspamd_logger.infox(rspamd_config,
                                            "found ip in keep_spam map, setting pre-result")
                        task:set_pre_result('accept',
                                            'ip matched with forward hosts')
                    end
                end
            end
        end
        ...
        if not redis_ret_user then
            rspamd_logger.infox(rspamd_config,
                                "cannot check keep_spam redis map")
        end
    end,
    priority = 19
})
...
rspamd_config:register_symbol({
    ...
    callback = function(task)
        ...
        if type(tls_tag) == 'nil' then
            ...
            task:set_milter_reply({
                add_headers = {
                    ['X-Last-TLS-Session-Version'] = tostring(tls_tag)
                }
            })
        end
    end,
    ...
    callback = function(task)
        ...
        if tagged_rcpt and tagged_rcpt[1].options and zynerone_domain then
            ...
            if action ~= 'no action' and action ~= 'greylist' then
                rspamd_logger.infox("skipping tag handler for action: %s",
                                    action)
                remove_moo_tag()
                return true
            end
            ...
            local function http_callback(err_message, code, body, headers)
                if body ~= nil and body ~= "" then
                    rspamd_logger.infox(rspamd_config,
                                        "expanding rcpt to \"%s\"", body)

                    local function tag_callback_subject(err, data)
                        if err or type(data) ~= 'string' then
                            rspamd_logger.infox(rspamd_config,
                                                "subject tag handler rcpt %s returned invalid or empty data (\"%s\") or error (\"%s\") - trying subfolder tag handler...",
                                                body, data, err)

                            local function tag_callback_subfolder(err, data)
                                if err or type(data) ~= 'string' then
                                    rspamd_logger.infox(rspamd_config,
                                                        "subfolder tag handler for rcpt %s returned invalid or empty data (\"%s\") or error (\"%s\")",
                                                        body, data, err)
                                    remove_moo_tag()
                                else
                            ...
                            if not redis_ret_subfolder then
                                rspamd_logger.infox(rspamd_config,
                                                    "cannot make request to load tag handler for rcpt")
                                remove_moo_tag()
                            end

                        else
                            rspamd_logger.infox(
                                "user wants subject modified for tagged mail")
            ...
            if rcpts and #rcpts == 1 then
                for _, rcpt in ipairs(rcpts) do
                    ...
                    if #rcpt_split == 2 then
                        if rcpt_split[1] == 'postmaster' then
                            rspamd_logger.infox(rspamd_config,
                                                "not expanding postmaster alias")
                            remove_moo_tag()
                        else
                            rspamd_http.request({
                                task = task,
    ...
    callback = function(task)
        ...
        local send_mail = function(task, bcc_dest)
            ...
            local function sendmail_cb(ret, err)
                if not ret then
                    ...
                    rspamd_logger.infox(rspamd_config, "BCC SMTP SUCCESS TO %s",
                                        bcc_dest)
                end
            end
        ...
        local function rcpt_callback(err_message, code, body, headers)
            if err_message == nil and code == 201 and body ~= nil then
                if action == 'no action' or action == 'add header' or action ==
                    'rewrite subject' then send_mail(task, body) end
            end
        end
        ...
        if rcpt_table then
            for _, e in ipairs(rcpt_table) do
                rspamd_logger.infox(rspamd_config,
                                    "checking bcc for rcpt address %s", e)
                rspamd_http.request({
                    task = task,
        ...
        if from_table then
            for _, e in ipairs(from_table) do
                rspamd_logger.infox(rspamd_config,
                                    "checking bcc for from address %s", e)
                rspamd_http.request({
                    task = task,
                    url = 'http://nginx:8081/bcc.php',
                    body = '',
...
rspamd_config:register_symbol({
    ...
    callback = function(task)
        ...
        local function redis_cb_user(err, data)
            ...
            if err or type(data) ~= 'string' then
                rspamd_logger.infox(rspamd_config,
                                    "dynamic ratelimit request for user %s returned invalid or empty data (\"%s\") or error (\"%s\") - trying dynamic ratelimit for domain...",
                                    uname, data, err)

                local function redis_key_cb_domain(err, data)
                    if err or type(data) ~= 'string' then
                        rspamd_logger.infox(rspamd_config,
                                            "dynamic ratelimit request for domain %s returned invalid or empty data (\"%s\") or error (\"%s\")",
                                            env_from_domain, data, err)
                    else
                        rspamd_logger.infox(rspamd_config,
                                            "found dynamic ratelimit in redis for domain %s with value %s",
                                            env_from_domain, data)
                        task:insert_result('DYN_RL', 0.0, data, env_from_domain)
                    end
                end

                local redis_ret_domain =
                    rspamd_redis_make_request(task, redis_params, -- connect params
                ...
                if not redis_ret_domain then
                    rspamd_logger.infox(rspamd_config,
                                        "cannot make request to load ratelimit for domain")
                end
            else
                rspamd_logger.infox(rspamd_config,
                                    "found dynamic ratelimit in redis for user %s with value %s",
                                    uname, data)
        ...
        if not redis_ret_user then
            rspamd_logger.infox(rspamd_config,
                                "cannot make request to load ratelimit for user")
        end
        return true
    end,
    flags = 'empty',
...
rspamd_config:register_symbol({
    ...
    callback = function(task)
        ...
        if from and
            (monitoring_hosts:get_key(from) or from == "watchdog@localhost") then
...
rspamd_config:register_symbol({
    ...
    callback = function(task)
        ...
        local function redis_cb_footer(err, data)
            if err or type(data) ~= 'string' then
                rspamd_logger.infox(rspamd_config,
                                    "domain wide footer request for user %s returned invalid or empty data (\"%s\") or error (\"%s\")",
                                    uname, data, err)
            else
                -- parse json string
                ...
                if not res then
                    rspamd_logger.infox(rspamd_config,
                                        "parsing domain wide footer for user %s returned invalid or empty data (\"%s\") or error (\"%s\")",
                                        uname, data, err)
                else
                    local footer = parser:get_object()

                    if footer and type(footer) == "table" and
                        (footer.html or footer.plain) then
                        rspamd_logger.infox(rspamd_config,
                                            "found domain wide footer for user %s: html=%s, plain=%s",
                                            uname, footer.html, footer.plain)
                        ...
                        if footer.html then
                            footer.html =
                                lua_util.jinja_template(footer.html,
                                                        replacements, true)
                        end
                        if footer.plain then
                            footer.plain =
                                lua_util.jinja_template(footer.plain,
                                                        replacements, true)
                        end

                        -- add footer
                        local out = {}
                        local rewrite = lua_mime.add_text_footer(task,
                                                                 footer.html,
                                                                 footer.plain) or
                                            {}
                        ...
                        local function rewrite_ct_cb(name, hdr)
                            if rewrite.need_rewrite_ct then
                                if name:lower() == 'content-type' then
                                    ...
                                    return
                                elseif name:lower() ==
                                    'content-transfer-encoding' then
                                    out[#out + 1] =
                                        string.format('%s: %s',
                        ...
                        rspamd_logger.infox(rspamd_config,
                                            "domain wide footer request for user %s returned invalid or empty data (\"%s\")",
                                            uname, data)
                    end
                end
            end
        ...
        if not redis_ret_footer then
            rspamd_logger.infox(rspamd_config,
                                "cannot make request to load footer for domain")
        end
